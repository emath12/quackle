...
class Quackle::V0LexiconInterpreter : public LexiconInterpreter {
  ...
  virtual void dawgAt(const unsigned char *dawg, int index, unsigned int &p,
                      Letter &letter, bool &t, bool &lastchild, bool &british,
    ...
    playability =
        (dawg[index + 4] << 16) + (dawg[index + 5] << 8) + (dawg[index + 6]);
  }
...
LexiconParameters::~LexiconParameters() { unloadAll(); }
...
void LexiconParameters::loadDawg(const string &filename) {
  ...
  fs::path directory =
      filePath.parent_path(); // Use parent_path to get the directory

  // Check if the directory exists and is accessible
  UVcout << "Listing files in directory: " << directory << endl;
  if (fs::exists(directory) && fs::is_directory(directory)) {
    for (const auto &entry : fs::directory_iterator(directory)) {
      UVcout << entry.path().string() << endl;
    }
  } else {
    UVcout << "Directory does not exist or is not accessible." << endl;
  }
  ...
  // Continue with loading DAWG as before
...
void LexiconParameters::loadGaddag(const string &filename) {
  ...
  // must create a local interpreter because dawg/gaddag versions might not
  // match
...
string LexiconParameters::logoFileName() const {
  ...
  if (colonPos == string::npos)
    return string();
  return QUACKLE_DATAMANAGER->makeDataFilename(
      "lexica", copyrightLine.substr(colonPos + 1), false);
}
