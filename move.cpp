...
bool operator==(const Move &move1, const Move &move2) {
  ...
  if (move1.action == move2.action &&
      move1.scoreAddition() == move2.scoreAddition()) {
    switch (move1.action) {
    case Quackle::Move::Place:
    case Quackle::Move::PlaceError:
      ret =
          (move1.horizontal == move2.horizontal &&
           move1.startrow == move2.startrow &&
           move1.startcol == move2.startcol && move1.tiles() == move2.tiles() &&
           move1.isChallengedPhoney() == move2.isChallengedPhoney());
      break;

    case Quackle::Move::UnusedTilesBonus:
    case Quackle::Move::UnusedTilesBonusError:
    case Quackle::Move::Exchange:
      ret = (Quackle::String::alphabetize(move1.tiles()) ==
             Quackle::String::alphabetize(move2.tiles()));
      break;
...
LetterString Move::usedTiles() const {
  return (m_isChallengedPhoney || action == BlindExchange)
             ? LetterString()
             : String::usedTiles(m_tiles);
}
...
UVString Move::xml() const {
  ...
  case PlaceError: {
    ...
    // restStream << "position=\"" << positionString() << "\" startrow=\"" <<
  ...
  if (includeTiles)
    xmlStream << " tiles=\""
              << QUACKLE_ALPHABET_PARAMETERS->userVisible(tiles()) << "\"";
  if (includeRest)
    xmlStream << " " << rest;
  if (includeScore)
...
UVString Move::toString() const {
  ...
  if (action == Quackle::Move::Pass)
    ...
    ss << "timepenalty " << score;
           ...
           action == Quackle::Move::UnusedTilesBonusError)
    ss << "(" << QUACKLE_ALPHABET_PARAMETERS->userVisible(m_tiles) << ")";
  else if (action == Quackle::Move::Place ||
           action == Quackle::Move::PlaceError) {
    ss << positionString();
    ss << " " << QUACKLE_ALPHABET_PARAMETERS->userVisible(m_tiles);
  }
...
UVString Move::debugString() const {
  ...
  ss << toString() << " (score = " << score << ", equity = " << equity
     << ", win% = " << win;
...
Move Move::createPlaceMove(int zeroIndexedRow, int zeroIndexedColumn,
...
Move Move::createChallengedPhoney(int zeroIndexedRow, int zeroIndexedColumn,
                                  bool horizontal, LetterString word) {
  Move move =
      createPlaceMove(zeroIndexedRow, zeroIndexedColumn, horizontal, word);
  move.setIsChallengedPhoney(true);
  return move;
}
...
bool MoveList::bagExistence(const Move &move1, const Move &move2) {
  return true;
}
