...
inline int HistoryLocation::playerId() const { return m_playerId; }

inline int HistoryLocation::turnNumber() const { return m_turnNumber; }
...
public:
  ...
  void kibitz(int nmoves = 10, int version = 0);
  ...
  int handleOverdraw(const LetterString &letters,
  ...
  PlayerList::const_iterator nextPlayerOfType(Player::PlayerType type,
                                              bool &found) const;

  // Returns the player with the abbreviated name specified, or the current
  // player if not found.
  PlayerList::const_iterator
  playerWithAbbreviatedName(const UVString &abbreviatedName, bool &found) const;
  ...
  bool canSetPlayerRackWithoutBagExpansion(int playerID,
  ...
  void setTileBonus(const UVString &player, const LetterString &allegedTiles,
...
inline Player &GamePosition::currentPlayer() { return *m_currentPlayer; }
...
inline void GamePosition::setBoard(const Board &board) { m_board = board; }

inline const Board &GamePosition::board() const { return m_board; }

inline Board &GamePosition::underlyingBoardReference() { return m_board; }

inline const Bag &GamePosition::bag() const { return m_bag; }

inline void GamePosition::setBag(const Bag &bag) { m_bag = bag; }
...
inline const PlayerList &GamePosition::players() const { return m_players; }

inline int GamePosition::spread() const { return spread(currentPlayer().id()); }

inline void GamePosition::replenishAndSetRack(const Rack &previousRack) {
  replenishAndSetRack(previousRack, currentPlayer());
}

inline bool GamePosition::gameOver() const { return m_gameOver; }

inline void GamePosition::setMoveMade(const Move &move) {
  m_moveMade = move;
  if (m_gameOver && move.action != Quackle::Move::UnusedTilesBonus &&
      move.action != Quackle::Move::UnusedTilesBonusError) {
    m_gameOver = false; // apparently the game isn't over...somebody's
                        // force-feeding us bad plays
    --m_turnNumber;
    m_moveMade.action = Quackle::Move::PlaceError;
    m_explanatoryNote =
        "Quackle says: Tiles were drawn out of order, leading to extra turns";
...
inline const Move &GamePosition::moveMade() const { return m_moveMade; }
...
inline void GamePosition::incrementNestedness() { ++m_nestedness; }
...
inline int GamePosition::turnNumber() const { return m_turnNumber; }

inline unsigned int GamePosition::nestedness() const { return m_nestedness; }
...
public:
  History() : /* a position in the past */ m_currentLocation(0, -1) {}
  ...
  const GamePosition &positionAt(const HistoryLocation &location,
                                 bool *exists = 0) const;
...
private:
  ...
  GamePosition &mutablePositionAt(const HistoryLocation &location,
                                  bool *exists = 0);
...
inline void History::setPlayers(const PlayerList &list) { m_players = list; }

inline const PlayerList &History::players() const { return m_players; }

inline const Player &History::lastPlayer() const {
  return back().playerOnTurn();
}

inline const GamePosition &History::lastPosition() const { return back(); }

inline GamePosition &History::lastPosition() { return back(); }
...
inline const PlayerList &Game::players() const { return m_positions.players(); }

inline bool Game::hasPositions() const { return !m_positions.empty(); }

inline const History &Game::history() const { return m_positions; }
...
inline void Game::resetCandidature() { currentPosition().resetMoveMade(); }
...
inline const UVString &Game::description() const { return m_description; }

inline void Game::setDescription(const UVString &description) {
  m_description = description;
}

inline const UVString &Game::title() const { return m_title; }

inline void Game::setTitle(const UVString &title) { m_title = title; }
