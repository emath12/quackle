...
Game::Game() : m_defaultComputerPlayer(0) { reset(); }
...
void Game::reset() {
  ...
  dummyPlayers.push_back(
      Player(MARK_UV("Dummy"), Quackle::Player::HumanPlayerType, 0));
...
void Game::associateComputerPlayer(int playerId,
...
Move Game::haveComputerPlay(ComputerPlayer *computerPlayer) {
  ...
  if (!computerPlayer)
    computerPlayer =
        this->computerPlayer(currentPosition().currentPlayer().id());
...
void Game::advanceToNoncomputerPlayer() {
  while (!currentPosition().gameOver() &&
         currentPosition().currentPlayer().type() == Player::ComputerPlayerType)
    haveComputerPlay();
}
...
GamePosition::GamePosition(const PlayerList &players)
    : m_players(players), m_currentPlayer(m_players.end()),
      m_playerOnTurn(m_players.end()), m_turnNumber(0), m_nestedness(0),
      m_scorelessTurnsInARow(0), m_gameOver(false),
      m_tilesOnRack(QUACKLE_PARAMETERS->rackSize()) {
  setEmptyBoard();
  resetMoveMade();
  resetBag();
  m_tilesInBag = m_bag.fullBagTileCount() -
                 (QUACKLE_PARAMETERS->rackSize() * int(m_players.size()));
}

GamePosition::GamePosition(const GamePosition &position)
    : m_players(position.m_players), m_moves(position.m_moves),
      m_moveMade(position.m_moveMade),
      m_committedMove(position.m_committedMove),
      m_turnNumber(position.m_turnNumber), m_nestedness(position.m_nestedness),
...
GamePosition::GamePosition()
    : m_currentPlayer(m_players.end()), m_playerOnTurn(m_players.end()),
      m_turnNumber(0), m_nestedness(0), m_scorelessTurnsInARow(0),
      m_gameOver(false) {
  setEmptyBoard();
  resetMoveMade();
  resetBag();
...
void GamePosition::kibitz(int nmoves, int version) {
...
void GamePosition::removeAllMoves() { m_moves.clear(); }
...
int GamePosition::handleOverdraw(const LetterString &letters,
                                 LetterString *throwback) const {
...
void GamePosition::ensureProperBag() const {
  ...
  for (Letter letter = 0; letter <= QUACKLE_ALPHABET_PARAMETERS->lastLetter();
       ++letter) {
    if (allTilesCounts[(int)(letter)] != fullCounts[(int)(letter)]) {
      ...
      if (debugString.empty()) {
        ...
        debugString +=
            MARK_UV("All tiles: ") + allTiles.toString() + MARK_UV("\n");
      }
      debugString += MARK_UV("Letter doesn't match: ") +
                     QUACKLE_ALPHABET_PARAMETERS->userVisible(letter) +
                     MARK_UV("\n");
    }
  }
  if (mismatch) {
...
void GamePosition::setEmptyBoard() { m_board.prepareEmptyBoard(); }
...
void GamePosition::replenishAndSetRack(const Rack &previousRack,
                                       Player &player) {
#ifdef VERBOSE_DEBUG_BAG
  UVcout << "replenishAndSetRack(" << previousRack << ", " << player << ")"
         << endl;
#endif
...
// TODO(jasonkb): this function is poorly implemented. Also it should return a
// reference.
...
void GamePosition::setPlayerRack(int playerID, const Rack &rack,
                                 bool adjustBag) {
  for (auto &it : m_players) {
    if (it.id() == playerID) {
      // restore bag
...
bool GamePosition::canSetCurrentPlayerRackWithoutBagExpansion(
    const Rack &rack) const {
  return canSetPlayerRackWithoutBagExpansion(currentPlayer().id(), rack);
}

bool GamePosition::canSetPlayerRackWithoutBagExpansion(int playerID,
                                                       const Rack &rack) const {
  (void)playerID;
  Bag someTiles(m_bag);
...
void GamePosition::setTileBonus(const UVString &player,
                                const LetterString &allegedTiles,
                                int allegedTileBonus) {
  bool found;
  Quackle::Move tileBonusMove =
      Quackle::Move::createUnusedTilesBonus(allegedTiles, allegedTileBonus);
  PlayerList::const_iterator currentPlayer =
      playerWithAbbreviatedName(player, found);
  if (currentPlayer == m_currentPlayer && m_gameOver) {
    ...
    // We computed this, but it differs from the actual tile bonus we're
    // requested to add
    tileBonusMove.action = Quackle::Move::UnusedTilesBonusError;
  } else if (allegedTileBonus > 0) {
    ++m_turnNumber;
    tileBonusMove.action =
        Quackle::Move::UnusedTilesBonusError; // we didn't empty a rack, but
                                              // somebody's claiming we did
  }
  ...
  m_gameOver = true;
  m_explanatoryNote =
      UVString("Quackle says: Bag wasn't empty or tiles drawn out of order");
}

void GamePosition::prepareForCommit() { setCommittedMove(moveMade()); }
...
void GamePosition::resetBag() { m_bag.prepareFullBag(); }
...
bool GamePosition::incrementTurn(const History *history) {
  ...
  if (m_currentPlayer != m_players.end()) {
    ...
    if (history) {
      ...
      const Quackle::PositionList positions(
          history->positionsFacedBy((*nextCurrentPlayer).id()));
      if (positions.size() > 0)
      ...
      while (m_tilesInBag > 0 &&
             m_tilesOnRack < QUACKLE_PARAMETERS->rackSize() &&
             m_moveMade.action == Move::Place) {
        m_tilesInBag--;
        m_tilesOnRack++;
      }
    }
    ...
    if ((m_moveMade.action == Move::Place &&
         m_moveMade.effectiveScore() == 0) ||
        m_moveMade.action == Move::Exchange ||
        m_moveMade.action == Move::BlindExchange ||
        m_moveMade.action == Move::Pass)
    ...
    if (!m_gameOver) {
      if (QUACKLE_PARAMETERS->numberOfScorelessTurnsThatEndsGame() >= 0 &&
          m_scorelessTurnsInARow >=
              QUACKLE_PARAMETERS->numberOfScorelessTurnsThatEndsGame() &&
          !m_board.isEmpty()) {
        // magic!
        ++m_turnNumber;
...
#ifdef VERBOSE_DEBUG_BAG
    UVcout << "removeLetters processing "
           << QUACKLE_ALPHABET_PARAMETERS->userVisible(*it) << endl;
#endif
...
#ifdef VERBOSE_DEBUG_BAG
        UVcout << "couldn't find any place to remove "
               << QUACKLE_ALPHABET_PARAMETERS->userVisible(*it) << " from!"
               << endl;
#endif
...
PlayerList::const_iterator
GamePosition::playerWithAbbreviatedName(const UVString &abbreviatedName,
                                        bool &found) const {
...
PlayerList::const_iterator
GamePosition::nextPlayerOfType(Player::PlayerType type, bool &found) const {
  PlayerList::const_iterator ret = m_currentPlayer;
  ++ret;
  while (true) {
    if (ret == m_players.end())
...
void GamePosition::adjustScoresToFinishPassedOutGame() {
  for (auto &it : m_players) {
    if (it == currentPlayer()) {
      m_moveMade =
          Move::createUnusedTilesBonus(it.rack().tiles(), -it.rack().score());
...
UVOStream &operator<<(UVOStream &o, const Quackle::GamePosition &position) {
  o << "------------" << endl;
  o << "Position [turnNumber = " << position.turnNumber() << ", current player "
    << position.currentPlayer() << endl;
...
const GamePosition &History::positionAt(const HistoryLocation &location,
                                        bool *exists) const {
  for (PositionList::const_reverse_iterator it = rbegin(); it != rend(); ++it) {
    if ((*it).playerOnTurn().id() == location.playerId() &&
        (*it).turnNumber() == location.turnNumber()) {
      if (exists)
        *exists = true;
      return *it;
    }
...
const GamePosition &History::nextPosition(bool *exists) const {
  ...
  for (PositionList::const_iterator it = begin(); it != end(); ++it) {
    ...
    if ((*it).playerOnTurn().id() == m_currentLocation.playerId() &&
        (*it).turnNumber() == m_currentLocation.turnNumber()) {
      found = true;
    }
  }
...
const GamePosition &History::nextPositionFacedBy(int playerID,
                                                 bool *exists) const {
  bool found = false;
  for (PositionList::const_iterator it = begin(); it != end(); ++it) {
...
bool Quackle::operator<(const HistoryLocation &hl1,
                        const HistoryLocation &hl2) {
  if (hl1.turnNumber() != hl2.turnNumber())
    return hl1.turnNumber() < hl2.turnNumber();
  return hl1.playerId() < hl2.playerId();
}

bool operator==(const Quackle::HistoryLocation &hl1,
                const Quackle::HistoryLocation &hl2) {
  return hl1.turnNumber() == hl2.turnNumber() &&
         hl1.playerId() == hl2.playerId();
}

UVOStream &operator<<(UVOStream &o,
                      const Quackle::HistoryLocation &historyLocation) {
  o << "location turn number " << historyLocation.turnNumber() << ", player id "
    << historyLocation.playerId();
  return o;
}
