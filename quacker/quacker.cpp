...
TopLevel::TopLevel(QWidget *parent)
    : QMainWindow(parent), m_listerDialog(0), m_letterbox(0), m_simViewer(0),
      m_plies(2), m_logania(0), m_modified(false) {
...
void TopLevel::introduceToUser() {
  ...
  QString statusText =
      QString::fromUtf8(QUACKLE_LEXICON_PARAMETERS->copyrightString().c_str());
  if (statusText.isEmpty())
    statusText = tr("Enjoy your quackling. Choose \"New game...\" from the "
                    "Game menu to begin.");
...
bool TopLevel::isPlayerOnTurnComputer() const {
  return m_game->currentPosition().playerOnTurn().type() ==
         Quackle::Player::ComputerPlayerType;
}

bool TopLevel::isCommitAllowed() const {
  return !m_game->currentPosition().gameOver() &&
         !(shouldOutcraftyCurrentPlayer() && isPlayerOnTurnComputer());
}
...
void TopLevel::commit() {
  ...
  if (!isCommitAllowed()) {
    statusMessage(
        tr("%1 is currently on turn so you cannot commit. Please wait.")
            .arg(QuackleIO::Util::uvStringToQString(
                m_game->currentPosition().playerOnTurn().name())));
    return;
  }
...
void TopLevel::overdraw() {
  bool ok = false;
  QString letters = QInputDialog::getText(
      this, tr("Handle overdraw - Quackle"),
      tr("Please input the letters that sit on the table faceup."),
  ...
  int validityFlags = m_game->currentPosition().handleOverdraw(
      QuackleIO::Util::nonBlankEncode(letters), &throwbackLetterString);

  if (validityFlags & Quackle::GamePosition::InvalidOverdrawNumber) {
    bool tryAgain =
        QMessageBox::question(
            this, tr("Verify Overdraw - Quackle"),
            dialogText(
                tr("Overdraw %1 does not contain at least %2 tiles, the "
                   "minimum number of tiles you should turn over. Try again?")
                    .arg(letters)
  ...
  if (validityFlags & Quackle::GamePosition::OverdrawnTilesNotUnseen) {
    bool tryAgain = QMessageBox::question(
                        this, tr("Verify Overdraw - Quackle"),
                        dialogText(tr("Tiles in overdraw %1 are not in the "
                                      "unseen pool. Try again?")
                                       .arg(letters)),
                        QMessageBox::Yes, QMessageBox::No) == QMessageBox::Yes;
    if (tryAgain)
      overdraw();
  ...
  QMessageBox::information(
      this, tr("Overdraw answer - Quackle"),
      dialogText(tr("Please put %1 back in the bag."))
          .arg(QuackleIO::Util::letterStringToQString(throwbackLetterString)));
}
...
bool TopLevel::askToCarryOn(const QString &text) {
  return QMessageBox::question(this, tr("Verify Play - Quackle"),
                               dialogText(text), QMessageBox::Yes,
...
void TopLevel::setCandidateMove(const Quackle::Move *move, bool *carryOnPtr) {
  if (carryOnPtr != nullptr)
    *carryOnPtr = true;
  if (!m_game->hasPositions() ||
      (move->action == Quackle::Move::Place && move->tiles().empty()))
    return;
    ...
    while (carryOn && validityFlags != Quackle::GamePosition::ValidMove) {
      if (validityFlags & Quackle::GamePosition::TooLateExchange) {
        carryOn = askToCarryOn(
            tr("Bag must contain at least %1 tiles for an exchange.")
                .arg(Quackle::DataManager::self()
                         ->parameters()
                         ->minimumTilesForExchange()));
      ...
      if (validityFlags & Quackle::GamePosition::InvalidTiles) {
        if (!m_game->currentPosition().currentPlayer().racksAreKnown()) {
          ...
          if (!validifiable) {
            carryOn = askToCarryOn(
                tr("%1 would need an impossible rack to make play %2; make "
                   "play anyway?")
                    .arg(QuackleIO::Util::uvStringToQString(
                        m_game->currentPosition().currentPlayer().name()))
          ...
          QMessageBox mb(
              QMessageBox::Question, tr("Verify Play"),
              tr("%1's rack does not include all tiles in %2; make play "
                 "anyway?")
                  .arg(QuackleIO::Util::uvStringToQString(
                      m_game->currentPosition().currentPlayer().name()))
                  .arg(QuackleIO::Util::moveToDetailedString(prettiedMove)));
          ...
          QPushButton *mb_unknownRacks = mb.addButton(
              tr("Assume unknown racks for this game"), QMessageBox::ApplyRole);
          mb.exec();
          carryOn = (mb.clickedButton() == mb_yes ||
                     mb.clickedButton() == mb_unknownRacks);
      ...
      if (validityFlags & Quackle::GamePosition::InvalidPlace) {
        carryOn = askToCarryOn(
            tr("%1 does not connect to other plays on board; make play anyway?")
                .arg(QuackleIO::Util::moveToDetailedString(prettiedMove)));
      ...
      if (validityFlags & Quackle::GamePosition::InvalidOpeningPlace) {
        carryOn = askToCarryOn(
            tr("Opening play %1 does not cover the star; make play anyway?")
                .arg(QuackleIO::Util::moveToDetailedString(prettiedMove)));
      ...
      if (validityFlags & Quackle::GamePosition::UnacceptableWord) {
        carryOn = askToCarryOn(
            tr("%1 forms an unacceptable word; make play anyway?")
                .arg(QuackleIO::Util::moveToDetailedString(prettiedMove)));

        if (carryOn) {
          if (QMessageBox::question(
                  this, tr("Challenge Decision - Quackle"),
                  dialogText(tr("If committed, should %1 be challenged off the "
                                "board?"))
                      .arg(QuackleIO::Util::moveToDetailedString(prettiedMove)),
                  QMessageBox::Yes, QMessageBox::No) == QMessageBox::Yes) {
  ...
  if (!m_game->currentPosition().currentPlayer().racksAreKnown() &&
      !m_game->currentPosition().currentPlayer().rack().contains(
          prettiedMove.usedTiles()) &&
      prettiedMove.action != Quackle::Move::BlindExchange) {
    m_game->currentPosition().setCurrentPlayerRack(
        Quackle::Rack(prettiedMove.usedTiles()));
  }
...
bool TopLevel::validifyMove(Quackle::Move &move) {
  ...
  if (move.action == Quackle::Move::Exchange) {
    ...
    for (Quackle::LetterString::const_iterator it = tiles.begin();
         it != tiles.end(); ++it) {
  ...
  if (move.action == Quackle::Move::Place) {
    if (!m_game->currentPosition().canSetCurrentPlayerRackWithoutBagExpansion(
            Quackle::Rack(tiles)))
      return false;
  }
...
void TopLevel::setRack(const Quackle::Rack &rack) {
  ...
  if (!m_game->currentPosition().canSetPlayerRackWithoutBagExpansion(
          m_game->currentPosition().currentPlayer().id(), rack)) {
    if (!askToCarryOn(
            tr("The rack %1 contains letters that are not available without "
               "modifying the tile distribution; do you wish to expand the bag "
               "to allow this?")
  ...
  m_game->currentPosition().setPlayerRack(
      m_game->currentPosition().currentPlayer().id(), rackToSet);
  m_simulator->currentPosition().setCurrentPlayerRack(rackToSet);
  updatePositionViews();

  statusMessage(
      tr("%1's rack set to %2.")
          .arg(QuackleIO::Util::uvStringToQString(
              m_game->currentPosition().currentPlayer().name()))
...
void TopLevel::goToHistoryLocation(const Quackle::HistoryLocation *location) {
  ...
  if (shouldOutcraftyCurrentPlayer() && isPlayerOnTurnComputer()) {
    statusMessage(
        tr("Please wait for %1 to play before looking at the history.")
            .arg(QuackleIO::Util::uvStringToQString(
                m_game->currentPosition().playerOnTurn().name())));
    return;
  }
...
TopLevel::createProgressBarForThread(OppoThread *thread) {
...
void TopLevel::removeProgressIndicators() {
  QMap<OppoThread *, OppoThreadProgressBar *>::const_iterator it =
      m_progressIndicators.constBegin();
...
void TopLevel::updatePositionViews() {
  ...
  m_simulateDetailsAction->setEnabled(
      !m_game->currentPosition().moves().empty());
  ...
  const QString assistiveText =
      tr("Click once or twice on the board, type, then press the Enter key.");

  if (commitAllowed && hasAMove)
    statusMessage(tr("Press a Commit button to play %1.")
                      .arg(QuackleIO::Util::moveToDetailedString(
                          m_game->currentPosition().moveMade())));
  else if (isPlayerOnTurnComputer())
    statusMessage(tr("%1 to play.")
                      .arg(QuackleIO::Util::uvStringToQString(
                          m_game->currentPosition().playerOnTurn().name())));
...
void TopLevel::updateMoveViews() {
  if (m_simulator->hasSimulationResults()) {
    const Quackle::MoveList &moveList =
        m_simulator->moves(/* prune */ true, /* sort by win */ true);
...
void TopLevel::updateHistoryViews() { emit historyChanged(m_game->history()); }
...
void TopLevel::initializeGame(const Quackle::PlayerList *players) {
  ...
  if (newPlayers.size() > 1) {
    ...
    while (m_firstPlayerName == prevFirst || m_firstPlayerName.empty()) {
      ...
      if (all_of(
              newPlayers.begin(), newPlayers.end(),
...
void TopLevel::open() {
  ...
  // QString getOpenFileName ( QWidget * parent = 0, const QString & caption =
  ...
  QString filename = QFileDialog::getOpenFileName(
      this, tr("Choose game file to open"), getInitialDirectory(),
      gameFileFilters(), &defaultFilter);
...
QString TopLevel::gameFileFilters() const {
  return tr("Game files (%1);;All files (*)")
      .arg(QuackleIO::Queenie::self()->filters().join(" "));
}
...
bool TopLevel::setupCheck() {
  ...
  if (!okay) {
    QMessageBox::warning(
        this, tr("Not Ready - Quackle"),
        dialogText(tr("Quackle cannot load its lexicon files.")));
  }
...
void TopLevel::plugIntoBaseMatrix(BaseView *view) {
  connect(view, SIGNAL(statusMessage(const QString &)), this,
...
void TopLevel::plugIntoMatrix(View *view) {
  ...
  connect(view, SIGNAL(setCandidateMove(const Quackle::Move *, bool *)), this,
          SLOT(setCandidateMove(const Quackle::Move *, bool *)));
  connect(view, SIGNAL(removeCandidateMoves(const Quackle::MoveList *)), this,
          SLOT(removeCandidateMoves(const Quackle::MoveList *)));
  connect(view, SIGNAL(commit()), this, SLOT(commit()));
  connect(view, SIGNAL(setRack(const Quackle::Rack &)), this,
          SLOT(setRack(const Quackle::Rack &)));
  connect(view, SIGNAL(setNote(const UVString &)), this,
          SLOT(setNote(const UVString &)));
}

void TopLevel::plugIntoPositionMatrix(View *view) {
  connect(this, SIGNAL(positionChanged(const Quackle::GamePosition *)), view,
          SLOT(positionChanged(const Quackle::GamePosition *)));
}

void TopLevel::plugIntoMoveMatrix(View *view) {
  connect(this, SIGNAL(movesChanged(const Quackle::MoveList *)), view,
          SLOT(movesChanged(const Quackle::MoveList *)));
}

void TopLevel::plugIntoHistoryMatrix(HistoryView *view) {
  plugIntoBaseMatrix(view);

  connect(view, SIGNAL(goToHistoryLocation(const Quackle::HistoryLocation *)),
          this, SLOT(goToHistoryLocation(const Quackle::HistoryLocation *)));

  connect(this, SIGNAL(historyChanged(const Quackle::History &)), view,
          SLOT(historyChanged(const Quackle::History &)));
}

QMessageBox::StandardButton TopLevel::askToSave() {
  return QMessageBox::warning(
      this, tr("Unsaved Moves - Quackle"),
...
void TopLevel::updateListerDialogWithRack() {
  if (m_listerDialog && m_game->hasPositions())
    m_listerDialog->setQuery(QuackleIO::Util::letterStringToQString(
        m_game->currentPosition().currentPlayer().rack().tiles()));
}
...
void TopLevel::firstPosition() {
  bool exists;
  const Quackle::GamePosition &firstPozzy =
      m_game->history().firstPosition(&exists);
  if (exists) {
...
void TopLevel::nextPosition() {
  bool exists;
  const Quackle::GamePosition &nextPozzy =
      m_game->history().nextPosition(&exists);
  if (exists) {
...
void TopLevel::previousPosition() {
  bool exists;
  const Quackle::GamePosition &previousPozzy =
      m_game->history().previousPosition(&exists);
  if (exists) {
...
void TopLevel::simulateToggled(bool startSimulation) {
  ...
  if (startSimulation) {
    switchToTab(ChoicesTabIndex);
    statusMessage(tr("Starting simulation. To stop the simulation, uncheck the "
                     "\"Simulate\" menu entry in the Move menu."));
...
void TopLevel::timeout() { UVcout << "toplevel::timeout" << endl; }
...
void TopLevel::logfileEnabled(bool on) { setLogfileEnabled(on); }
...
bool TopLevel::isLogfileEnabled() const { return m_logfileEnable->isChecked(); }

QString TopLevel::userSpecifiedLogfile() const { return m_logfileEdit->text(); }
...
void TopLevel::logfileChanged() {
  m_simulator->setLogfile(QuackleIO::Util::qstringToStdString(logfile()),
                          /* append */ true);
}
...
void TopLevel::chooseLogfile() {
  ...
  fileDialog->setDirectory(
      userSpecifiedLogfile().isEmpty()
          ? QDir::currentPath()
          : QFileInfo(userSpecifiedLogfile()).absolutePath());
...
QString TopLevel::partialOppoRack() const {
  return isPartialOppoRackEnabled() ? userSpecifiedPartialOppoRack()
                                    : QString("");
}
...
void TopLevel::partialOppoRackChanged() {
  ...
  Quackle::LetterString oppoTiles =
      m_game->currentPosition().currentPlayer().rack().tiles();
  Quackle::LetterString doubleTiles = rack.tiles();
  for (Quackle::LetterString::const_iterator it = oppoTiles.begin();
       it != oppoTiles.end(); ++it) {
  ...
  if (!m_game->currentPosition().canSetPlayerRackWithoutBagExpansion(
          m_game->currentPosition().currentPlayer().id(), doubleRack)) {
    QMessageBox::warning(
        this, tr("Wrackful Rack - Quackle"),
        dialogText(
            tr("The rack %1 contains letters that are not possible for "
               "opponent to hold.")
                .arg(QuackleIO::Util::letterStringToQString(rack.tiles()))));
...
void TopLevel::updateSimViews() {
  m_simulatorWidget->setTitle(
      m_simulator->hasSimulationResults()
          ? tr("Simulation: %2 iterations").arg(m_simulator->iterations())
          : tr("Simulation"));
...
void TopLevel::loadFile(const QString &filename) {
  ...
  if (!file.exists()) {
    QMessageBox::critical(
        this, tr("Error Loading Game File - Quackle"),
        dialogText(tr("Filename %1 does not exist.")).arg(filename));
    return;
  }

  if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
    QMessageBox::critical(this, tr("Error Loading Game File - Quackle"),
                          dialogText(tr("%1 cannot be opened.")).arg(filename));
    return;
  }

  QuackleIO::Logania *logania =
      QuackleIO::Queenie::self()->loganiaForFile(filename);
  if (logania == 0) {
    QMessageBox::critical(
        this, tr("Error Loading Game File - Quackle"),
        dialogText(tr("Sorry, %1 is in a format Quackle cannot read."))
            .arg(filename));
    file.close();
    return;
  }
...
void TopLevel::saveAs() {
  ...
  QString filename = QFileDialog::getSaveFileName(
      this, tr("Choose file to which to save game"), getInitialDirectory(),
      gameFileFilters(), &defaultFilter);
...
void TopLevel::reportAs(Quackle::ComputerPlayer *player) {
  QString filename = QFileDialog::getSaveFileName(
      this, tr("Choose file to which to write report"), getInitialDirectory());
  ...
  if (!filename.isEmpty()) {
    ...
    if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
      QMessageBox::critical(
          this, tr("Error Writing File - Quacker"),
          dialogText(tr("Could not open %1 for writing.")).arg(filename));
      return;
    }
...
void TopLevel::htmlReport() {
  ...
  QString filename = QFileDialog::getSaveFileName(
      this, tr("Choose HTML file to which to write report - Quackle"),
      getInitialDirectory(), "HTML Files (*.html)");
...
void TopLevel::graphicalReport() {
  ...
  QString directory = QFileDialog::getExistingDirectory(
      this, tr("Choose directory to which to write report and graphics"),
      getInitialDirectory());
...
void TopLevel::writeFile(const QString &filename) {
  ...
  QuackleIO::Logania *logania =
      m_logania ? m_logania : QuackleIO::Queenie::self()->defaultLogania();
...
void TopLevel::computerPlayerDone() {
  ...
  for (QList<OppoThread *>::iterator it = m_oppoThreads.begin();
       it != m_oppoThreads.end();) {
    if ((*it)->isFinished()) {
      removeProgressIndicator(*it);

      if ((*it)->position().playerOnTurn().positionallyEqual(
              m_game->currentPosition().playerOnTurn())) {
        moves = (*it)->moves();
      } else {
        UVcout << "Player on turn in position is not positionally equal to "
                  "current player"
               << endl;
...
void TopLevel::showToHuman() {
  ...
  if (m_game->currentPosition().gameOver()) {
    ...
    statusMessage(tr("Game over. Double-click an entry in the History table to "
                     "perform postmortem."));
  } else {
    pause(false);
  }
...
QString TopLevel::gameTitle() {
  ...
  if (players.size() == 0)
    ...
    ret = tr("Game between %1 and friends")
              .arg(QuackleIO::Util::uvStringToQString(players.front().name()));
...
void TopLevel::createMenu() {
  ...
  m_commitTopChoiceAction =
      new QAction(tr("&Commit top statically-evaluated choice"), this);
  m_commitTopChoiceAction->setShortcut(tr("Ctrl+T"));
  m_commitTopChoiceAction->setEnabled(false);
  connect(m_commitTopChoiceAction, SIGNAL(triggered()), this,
          SLOT(commitTopChoice()));
  ...
  for (Quackle::PlayerList::const_reverse_iterator it =
           QUACKLE_COMPUTER_PLAYERS.rbegin();
       it != QUACKLE_COMPUTER_PLAYERS.rend(); ++it) {
    if (!(*it).computerPlayer()->isUserVisible())
      continue;
    KibitzerListener *listener =
        new KibitzerListener((*it).computerPlayer(), this);

    QAction *kibitzAction =
        new QAction(tr("Ask %1 for choices")
                        .arg(QuackleIO::Util::uvStringToQString((*it).name())),
                    this);
    kibitzAction->setIconText(
        tr("Ask %1").arg(QuackleIO::Util::uvStringToQString((*it).name())));
    connect(kibitzAction, SIGNAL(triggered()), listener,
            SLOT(kibitzTriggered()));

    QAction *reportAction =
        new QAction(tr("Ask %1 for a full-game report")
                        .arg(QuackleIO::Util::uvStringToQString((*it).name())),
                    this);
    connect(reportAction, SIGNAL(triggered()), listener,
            SLOT(reportTriggered()));

    connect(listener, SIGNAL(kibitzAs(Quackle::ComputerPlayer *)), this,
            SLOT(kibitzAs(Quackle::ComputerPlayer *)));
    connect(listener, SIGNAL(reportAs(Quackle::ComputerPlayer *)), this,
            SLOT(reportAs(Quackle::ComputerPlayer *)));
  ...
  connect(m_firstPositionAction, SIGNAL(triggered()), this,
          SLOT(firstPosition()));

  m_previousPositionAction =
      new QAction(tr("&Back to previous position"), this);
  ...
  connect(m_previousPositionAction, SIGNAL(triggered()), this,
  ...
  connect(m_nextPositionAction, SIGNAL(triggered()), this,
          SLOT(nextPosition()));
  ...
  connect(m_simulateAction, SIGNAL(toggled(bool)), this,
  ...
  connect(m_simulateDetailsAction, SIGNAL(triggered()), this,
          SLOT(showSimulationDetails()));

  m_simulateClearAction = new QAction(tr("Clear simulation results"), this);
  m_simulateClearAction->setEnabled(false);
  connect(m_simulateClearAction, SIGNAL(triggered()), this,
          SLOT(clearSimulationResults()));
...
#ifdef ENABLE_GRAPHICAL_REPORT
  m_graphicalReportAction =
      new QAction(tr("Slow full-game HTML report with images"), this);
  m_graphicalReportAction->setEnabled(false);
  connect(m_graphicalReportAction, SIGNAL(triggered()), this,
          SLOT(graphicalReport()));
...
void TopLevel::createWidgets() {
  ...
  connect(m_ignoreOpposCheck, SIGNAL(stateChanged(int)), this,
          SLOT(ignoreOpposChanged()));

  m_showDetailsButton = new QPushButton(tr("&Details"));
  connect(m_showDetailsButton, SIGNAL(clicked()), this,
          SLOT(showSimulationDetails()));
  ...
  connect(m_partialOppoRackEnable, SIGNAL(toggled(bool)), this,
          SLOT(partialOppoRackEnabled(bool)));
  ...
  connect(m_partialOppoRackEdit, SIGNAL(textEdited(const QString &)), this,
          SLOT(partialOppoRackChanged()));
  ...
  connect(m_logfileEnable, SIGNAL(toggled(bool)), this,
          SLOT(logfileEnabled(bool)));
  ...
  connect(m_logfileEdit, SIGNAL(editingFinished()), this,
          SLOT(logfileChanged()));
...
QString TopLevel::playerString() const {
  ...
  Quackle::PlayerList players =
      m_game->currentPosition().endgameAdjustedScores();
  if (players.empty())
    return tr("No game");
  ...
  for (Quackle::PlayerList::const_iterator it = players.begin(); it != end;
       ++it, ++i) {
    if (!begin)
      ret += tr(", ");
...
void TopLevel::showAscii() {
  ...
  int result = QMessageBox::question(
      this, tr("Plaintext board - Quackle"), QString("<pre>%1</pre>").arg(text),
      tr("&Write to file"), tr("Copy to clip&board"), tr("&OK"), 1);

  switch (result) {
  case 0: {
    QString filename = QFileDialog::getSaveFileName(
        this, tr("Choose file to print plaintext board to - Quackle"),
        getInitialDirectory());
...
void TopLevel::writeAsciiToFile(const QString &text, const QString &filename) {
  ...
  if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
    QMessageBox::critical(
        this, tr("Error Writing File - Quackle"),
        dialogText(tr("Could not open %1 for writing.")).arg(file.fileName()));
    return;
  }
...
void TopLevel::print() {
  ...
  QString filename = QFileDialog::getSaveFileName(
      this, tr("Choose file to print to - Quackle"), m_filename + ".html");
...
void TopLevel::firstTimeRun() {
  ...
  QMessageBox::information(
      this, tr("Welcome - Quackle"),
      dialogText(
          tr("Welcome to Quackle! To get started, configure a board by "
             "clicking Add Board in the Settings tab, add some bonus squares, "
             "and then start a new game. Also check out the Helpful Hints from "
             "the Help menu, and keep your eye out for the aidant messages in "
             "the status bar at the bottom of the window.")));
}

void TopLevel::about() {
  QString aboutText =
      tr("<p><b>Quackle</b> 1.0.4.1 is a crossword game playing, analysis, and "
         "study tool. Visit the Quackle homepage at <tt><a "
         "href=\"http://quackle.org\">http://quackle.org</a></tt> for more "
         "information.</p>"
         "<p>Quackle was written by Jason Katz-Brown, John O'Laughlin, John "
         "Fultz, Matt Liberty, and Anand Buddhdev. We thank the anonymous "
         "donor who made this software free.</p>"
         ...
         "</ul>"
         "<p>Quackle is free, open-source software licensed under the terms of "
         "the GNU General Public License Version 3. See</p>"
         "<p><tt><a "
         "href=\"http://quackle.org/LICENSE\">http://quackle.org/LICENSE</a></"
         "tt></p>"
  ...
  if (file) {
    ...
    while (!line.isNull()) {
      ...
      if (startPos != -1 && startPos + 1 < line.size()) {
        ...
        if (line.indexOf("copyright", 0, Qt::CaseInsensitive) != -1 ||
            line.indexOf(QChar(0xA9)) != -1)
...
void TopLevel::saveSettings() {
  ...
  settings.setValue("quackle/partialopporackenabled",
                    isPartialOppoRackEnabled());
...
void TopLevel::loadSettings() {
  ...
  m_initialDirectory =
      settings.value("quackle/initial-directory", QString("")).toString();
  ...
  if (settings.contains("quackle/splitter-sizes"))
    m_splitter->restoreState(
        settings.value("quackle/splitter-sizes").toByteArray());
  ...
  m_ignoreOpposCheck->setChecked(
      settings.value("quackle/ignoreoppos", false).toBool());

  m_logfileEdit->setText(
      settings.value("quackle/logfile", QString("")).toString());
  const bool logfileEnabled =
      settings.value("quackle/logfileEnabled", false).toBool();
  m_logfileEnable->setChecked(logfileEnabled);
  setLogfileEnabled(logfileEnabled);

  m_partialOppoRackEdit->setText(
      settings.value("quackle/partialopporack", QString("")).toString());
  const bool partialOppoRackEnabled =
      settings.value("quackle/partialopporackenabled", false).toBool();
...
KibitzerListener::KibitzerListener(Quackle::ComputerPlayer *computerPlayer,
                                   QObject *parent)
    : QObject(parent), m_computerPlayer(computerPlayer),
      m_slowPlayerWarningTriggered(false) {}
...
bool KibitzerListener::slownessCheck() { return true; }
