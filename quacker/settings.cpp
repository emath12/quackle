...
Settings *Settings::self() { return m_self; }
...
Settings::Settings(QWidget *parent)
  ...
  if (CFBundleGetMainBundle()) {
    CFURLRef dataUrlRef = CFBundleCopyResourceURL(CFBundleGetMainBundle(),
                                                  CFSTR("data"), NULL, NULL);
    if (dataUrlRef) {
      CFStringRef macPath =
          CFURLCopyFileSystemPath(dataUrlRef, kCFURLPOSIXPathStyle);
      size_t sizeOfBuf =
          CFStringGetMaximumSizeOfFileSystemRepresentation(macPath);
  ...
  if (QFile::exists("data")) {
    ...
    if (!directory.cd("data") || !directory.cd("../data"))
      QMessageBox::critical(
          0, tr("Error Initializing Data Files - Quacker"),
          tr("<p>Could not open data directory. Quackle will be useless. Try "
             "running the quacker executable with quackle/quacker/ as the "
             "current directory.</p>"));
...
void Settings::createGUI() {
  ...
  connect(m_lexiconNameCombo, SIGNAL(activated(int)), this,
          SLOT(lexiconChanged(int)));
  ...
  connect(m_alphabetNameCombo, SIGNAL(activated(int)), this,
          SLOT(alphabetChanged(int)));
  ...
  connect(m_themeNameCombo, SIGNAL(activated(int)), this,
  ...
  connect(m_boardNameCombo, SIGNAL(activated(int)), this,
...
void Settings::load() {
  m_lexiconNameCombo->setCurrentIndex(m_lexiconNameCombo->findText(
      QString::fromUtf8(QUACKLE_LEXICON_PARAMETERS->lexiconName().c_str())));
  if (m_lexiconNameCombo->currentIndex() == -1)
    m_lexiconNameCombo->setCurrentIndex(m_lexiconNameCombo->findText(
        QString::fromUtf8(QUACKLE_LEXICON_PARAMETERS->lexiconName().c_str()) +
        "*"));
  m_lastGoodLexiconValue = m_lexiconNameCombo->currentIndex();
  m_alphabetNameCombo->setCurrentIndex(m_alphabetNameCombo->findText(
      QString::fromUtf8(QUACKLE_ALPHABET_PARAMETERS->alphabetName().c_str())));
  m_themeNameCombo->setCurrentIndex(m_themeNameCombo->findText(m_themeName));
  m_boardNameCombo->setCurrentIndex(m_boardNameCombo->findText(
      QuackleIO::Util::uvStringToQString(QUACKLE_BOARD_PARAMETERS->name())));
...
void Settings::preInitialize() {
  ...
  QUACKLE_DATAMANAGER->setComputerPlayers(
...
void Settings::initialize() {
  ...
  QString lexiconName =
      settings.value("quackle/settings/lexicon-name", QString("nwl18"))
          .toString();
  ...
  setQuackleToUseAlphabetName(
      settings.value("quackle/settings/alphabet-name", QString("english"))
          .toString());
  setQuackleToUseThemeName(
      settings.value("quackle/settings/theme-name", QString("traditional"))
          .toString());
  setQuackleToUseBoardName(
      settings.value("quackle/settings/board-name", QString("")).toString());
}

void Settings::setGaddagLabel() {
  QString gaddagLabelString;
  if (!QUACKLE_LEXICON_PARAMETERS->hasGaddag()) {
    gaddagLabelString = tr(
        "Lexicon database is not up to date.  Press the button to begin "
        "building the database.  This may take several minutes to complete.");
...
void Settings::buildGaddag() {
  const string gaddagFile(QUACKLE_DATAMANAGER->makeDataFilename(
      "lexica", QUACKLE_LEXICON_PARAMETERS->lexiconName() + ".gaddag", true));
  ...
  if (wordCount < QUACKLE_MAX_GADDAG_WORDCOUNT) {
    setGaddagLabel(
        QString(tr("Lexicon total: %1 words.  Compressing...")).arg(wordCount));
    factory.generate();
    setGaddagLabel(QString(tr("Lexicon total: %1 words.  Writing to disk..."))
                       .arg(wordCount));
    factory.writeIndex(gaddagFile);
    QUACKLE_LEXICON_PARAMETERS->loadGaddag(gaddagFile);
    setGaddagLabel();
  } else
    setGaddagLabel(tr("Your lexicon is too large to be represented using the "
                      "internal database format.  Operation aborted."));
}

void Settings::pushIndex(GaddagFactory &factory, Quackle::LetterString &word,
                         int index, int &wordCount) {
  unsigned int p;
  ...
  do {
    QUACKLE_LEXICON_PARAMETERS->dawgAt(index, p, letter, t, lastchild, british,
                                       playability);
...
void Settings::setQuackleToUseLexiconName(const QString &lexiconName) {
  ...
  if (QUACKLE_LEXICON_PARAMETERS->lexiconName() != lexiconNameStr) {
    ...
    string dawgFile = Quackle::LexiconParameters::findDictionaryFile(
        lexiconNameStr + ".dawg");
    if (dawgFile.empty()) {
      UVcout << "Dawg for lexicon '" << lexiconNameStr << "' does not exist."
             << endl;
    ...
    string gaddagFile = Quackle::LexiconParameters::findDictionaryFile(
        lexiconNameStr + ".gaddag");
    if (gaddagFile.empty()) {
      UVcout << "Gaddag for lexicon '" << lexiconNameStr << "' does not exist."
             << endl;
      QUACKLE_LEXICON_PARAMETERS->unloadGaddag();
    } else
      QUACKLE_LEXICON_PARAMETERS->loadGaddag(gaddagFile);

    // Dirty test to see if we're working with an English-like dictionary, and
    // if so, beef up strategy files with twl06 ones (until I can start
    // generating better).  It's an imperfect test...it captures the ODS
    // dictionary, for example, which seems pretty wrong.  But I don't want to
    // hard-code lexicon names here, so this is about as good as I can do.
    ...
    m_copyrightLabel->setText(QString::fromUtf8(
        QUACKLE_LEXICON_PARAMETERS->copyrightString().c_str()));
    setGaddagLabel();
  }
}
...
void Settings::setQuackleToUseAlphabetName(const QString &alphabetName) {
  ...
  if (QUACKLE_ALPHABET_PARAMETERS->alphabetName() != alphabetNameStr) {
    QString alphabetFileStr = QuackleIO::Util::stdStringToQString(
        Quackle::AlphabetParameters::findAlphabetFile(alphabetNameStr));

    QuackleIO::FlexibleAlphabetParameters *flexure =
        new QuackleIO::FlexibleAlphabetParameters;
    flexure->setAlphabetName(alphabetNameStr);
    if (flexure->load(alphabetFileStr)) {
      if (flexure->length() != QUACKLE_ALPHABET_PARAMETERS->length() &&
          QUACKLE_ALPHABET_PARAMETERS->alphabetName() != "default") {
        QMessageBox::warning(
            this, tr("Alphabet mismatch - Quackle"),
            QString("<html>%1</html>")
                .arg(tr("%1 has a different number of letters than %2, so "
                        "please start a new game or else Quackle will crash or "
                        "act strangely.")
                         .arg(QuackleIO::Util::stdStringToQString(
                             flexure->alphabetName()))
                         .arg(QuackleIO::Util::stdStringToQString(
                             QUACKLE_ALPHABET_PARAMETERS->alphabetName()))));
      }
...
void Settings::setQuackleToUseBoardName(const QString &boardName) {
  ...
  if (boardName.isEmpty() || !settings.contains(boardName))
    ...
    QByteArray boardParameterBytes =
        qUncompress(settings.value(boardName).toByteArray());
    string boardParameterBuf;
    boardParameterBuf.assign((const char *)boardParameterBytes,
                             boardParameterBytes.size());
    istringstream boardParameterStream(boardParameterBuf);

    QUACKLE_DATAMANAGER->setBoardParameters(
        Quackle::BoardParameters::Deserialize(boardParameterStream));
  }
  QUACKLE_BOARD_PARAMETERS->setName(
      QuackleIO::Util::qstringToString(boardName));
  loadBoardNameCombo();
}
...
void Settings::alphabetChanged(int alphabetIndex) {
  ...
  // if (m_alphabetNameCombo->currentIndex() == m_alphabetNameCombo->count() -
  // 1)
  // {
  // 	editAlphabet();
  // 	return;
...
void Settings::themeChanged(int themIndex) {
  ...
  CustomQSettings settings;
  settings.setValue("quackle/settings/theme-name",
                    m_themeNameCombo->currentText());
...
void Settings::addBoard() {
  ...
  if (dialog.exec()) {
    ...
    QByteArray boardParameterBytes =
        qCompress((const uchar *)boardParameterStream.str().data(),
...
void Settings::editBoard() {
  ...
  QUACKLE_BOARD_PARAMETERS->setName(
      QuackleIO::Util::qstringToString(oldBoardName));
  ...
  if (dialog.exec()) {
    ...
    QByteArray boardParameterBytes =
        qCompress((const char *)boardParameterStream.str().data(),
                  int(boardParameterStream.str().size()));
...
void Settings::editLexicon() {
  ...
  if (dialog.exec()) {
    populateComboFromFilenames(m_lexiconNameCombo, "lexica", ".dawg",
                               "lexicon");
    qApp->processEvents();
    if (dialog.itemWasDeleted()) {
      ...
      m_lexiconNameCombo->setCurrentIndex(
          m_lexiconNameCombo->findText(name + "*"));
    }
    load();
  }
}
...
void Settings::populateComboFromFilenames(QComboBox *combo, const QString &path,
