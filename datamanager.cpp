...
DataManager::DataManager()
    : m_evaluator(0), m_parameters(0), m_alphabetParameters(0),
      m_boardParameters(0), m_lexiconParameters(0), m_strategyParameters(0) {
  m_self = this;
  setAppDataDirectory(".");
  setUserDataDirectory(".");

  seed_seq session_seed = {(unsigned)random_device{}(),
...
bool DataManager::isGood() const { return m_lexiconParameters->hasSomething(); }
...
void DataManager::setAlphabetParameters(
...
void DataManager::setStrategyParameters(
...
string DataManager::findDataFile(const string &subDirectory,
                                 const string &lexicon, const string &file) {
  string fname = makeDataFilename(subDirectory, lexicon, file, true);
  cerr << "Attempting to load file: " << fname << endl; // Log the first attempt
  if (!fileExists(fname)) {
    fname = makeDataFilename(subDirectory, lexicon, file, false);
    cerr << "File not found. Trying alternative: " << fname
         << endl; // Log if the first attempt fails
  }
  if (!fileExists(fname) && lexicon.substr(0, 3) == "csw") {
    fname = makeDataFilename(subDirectory, "csw", file, false);
    cerr << "Trying csw lexicon: " << fname
         << endl; // Log if trying the csw lexicon
  }
  if (!fileExists(fname)) {
    fname = makeDataFilename(subDirectory, m_backupLexicon, file, false);
    cerr << "Trying backup lexicon: " << fname
  ...
  if (!fileExists(fname)) {
    cerr << "File not found after all attempts."
         << endl; // Log if all attempts fail
    fname = string();
  }
...
bool DataManager::hasUserDataFile(const string &subDirectory,
                                  const string &file) {
  string fname = makeDataFilename(subDirectory, file, true);
  return fileExists(fname);
}

string DataManager::makeDataFilename(const string &subDirectory,
                                     const string &lexicon, const string &file,
                                     bool user) {
  return (user ? m_userDataDirectory : m_appDataDirectory) + "/" +
         subDirectory + "/" + lexicon + "/" + file;
}
