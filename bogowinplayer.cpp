...
MoveList SmartBogowin::moves(int nmoves) {
  ...
  // This can be done efficiently by making a big "rack" out of the bag (with
  // the computer player's own rack removed) and generating moves from that big
  // "rack" with some smarts added to the move generator to not create moves
  // that have so many tiles not in the opp's play that they wouldn't have been
  // possible from any of the racks we're looking for. Make a ProbableRackList
  // of racks from which the opp's play is best or nearly (really what we're
  // looking for is least bad). Most heavily weight the leaves for which the
  // play is as close to optimal as possible (an x-point static mistake), and
  // let the weights taper off to zero as the mistakes approach say x+7.
  //
  ...
  if (m_parameters.inferring) {
    ...
    if (numPlayers == 2) {
      ...
      GamePosition previous =
          m_simulator.history().previousPosition(&hasPreviousPosition);
  ...
  // if (m_parameters.game.history().size() > 1) {
  //   	// if a game has been passed, then commence with board driven behavior.
  // 	Bag currentBag = m_parameters.game.currentPosition().bag();
  // 	MoveList moves_from_kibitz = currentPosition().m_moves;

  // 	char countsArray[26] = {0};
  // 	currentBag.letterCounts(countsArray);
  // 	for (Move move : moves_from_kibitz) {
  // 		if (move.isBingo) {
  // 			continue;
  // 		}

  // 		LetterString lettersInMove = move.usedTiles()
  ...
  for (++it; it != staticMoves.end(); ++it) {
    ...
    // UVcout << "best move: " << best << " with " << bestbp  << " bogopoints."
    // << endl;
    ...
    // UVcout << "we just simmed " << move << "; bogopoints: " << movebp <<
    // endl;
    ...
    if (movebp + 1.96 * 35.0 / sqrt((double)minIterations()) > bestbp) {
      ...
      // UVcout << "sim it some more: " << move2 << " bogopoints: " << movebp <<
      // endl;
    ...
    if (stopwatch.exceeded(m_parameters.secondsPerTurn)) {
      // UVcout << "Bogowinplayer stopwatch exceeded its limit " <<
      // m_parameters.secondsPerTurn << ". Returning early." << endl;
      goto sort_and_return;
    }
  }
